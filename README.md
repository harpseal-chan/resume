...
## 職務経歴
※現在、社名は伏せて記載しております。詳細な職務経歴につきましては、選考の際に別途お伝えいたします。

### ■Web受託開発企業 (2024.4 - 2025.8)

#### 会計事務所向け業務管理システム開発 （2024.4 - 2025.8）
会計事務所の業務効率化を目的としたクラウドシステムを開発。
システムのメイン機能である「カレンダー」において、フロントエンドからバックエンドまで一貫した実装を担当しました。  
また、チーム内でのコードレビューにも積極的に携わり、コードの品質向上とチーム全体の開発効率化に貢献しました。
...
FullCalendarをベースに、予定の表示・登録・編集・削除などの基本機能を網羅しつつ、要件に基づいた高度な表示制御を実装しました。
- ロジック面： プルダウンで選択された複数メンバーのスケジュールを、選択順に動的に表示・制御する機能を担当。  
  選択順の保持、およびバツボタンによる特定メンバーの除外と、それに伴う再レンダリング制御を実装し、直感的な操作感を実現しました。
- UI面： FullCalendar公式ドキュメントを読み込み、細かなオプション設定やイベント制御をカスタマイズすることで、外部ライブラリをプロジェクト固有の複雑な仕様に合わせ最適化しました。

**セキュリティ脆弱性の解消および大規模リファクタリング**  
脆弱性検査により、バックエンドの内部エラー詳細がGraphQLのレスポンスに含まれる課題が判明。  
これに対してエラーハンドリングを共通化・標準化し、計82ファイルのGraphQLリゾルバおよびミューテーションをリファクタリングしました。  
その結果、脆弱性検査をクリアし、システムの堅牢性向上に貢献しました。

**コードレビューによる開発効率の向上**  
チーム全体の開発サイクル高速化のため、GitLabのSuggested Changes機能を積極的に活用。  
typoや軽微な修正だけでなく、具体的なコード案を提示することでレビュイーの修正工数を削減しました。  
また、バグがある場合は根本的な原因と具体的な解消策までセットで提案することで、再発防止とチームの技術レベル向上に貢献しました。

### ■エンベデッドシステムソリューション企業（2020.04 - 2024.3）

組み込みシステムエンジニアとして、主にSSDの管理・性能評価CLIツールとBluetooth通信Androidアプリケーションの開発に従事し、要件定義から設計、実装、テストまで一貫したシステム開発工程に携わりました。

<details>
<summary>業務内容詳細</summary>
...
|Ruby| 2年 | 実務で使える（Webアプリ開発） |
|HTML / CSS| 2年 | 実務で使える（Webアプリ開発） |
|JavaScript| 2年 | 実務で使える（Webアプリ開発） |
|TypeScript| 1年 | 実務で使える（Webアプリ開発） |

### フレームワーク・ライブラリ
| フレームワーク・ライブラリ | 期間 | レベル |
|--|-----|-----|
|Ruby on Rails| 2年 | 実務で使える |
|React| 1年 | 実務で使える |
